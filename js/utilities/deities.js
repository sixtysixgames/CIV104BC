...
function tickWalk() {
    ...
    for (let i = 0; i < civData.walk.rate; ++i) {
        ...
        gameLog("A " + target + " has been sacrificed.")
    }
...
function iconoclasm(index) {
    ...
    //give coins
    let coins = Math.floor(Math.pow(curCiv.deities[index].maxDev, 1 / 1.25)) * 100;
    civData.coins.owned += coins;
    gameLog("You gain " + coins + " coins");
    //remove the deity
...
export {
    getCurDeityDomain, getCurrentAltarId, getAltarsOwned, typeToId, idToType, getDeityRowText, makeDeitiesTables, raiseDead, summonShade, selectDeity,
wickerman, walk, tickWalk, pestControl, iconoclasmList, iconoclasm, smiteMob, smite, glory, grace, doPestControl, tickGlory, doThrone, tickGrace };