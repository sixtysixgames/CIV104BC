...
function addUpgradeRows() {
    ...
    // Deity granted powers Prayers
    powerData.forEach(function (elem) {
        if (elem.subType == subTypes.prayer) {
            setPantheonUpgradeRowText(elem);
            let stubElem = document.getElementById(elem.id);
            stubElem.onmousedown = getPrayerFunction(elem.id);
        }
    });
...
function getPrayerFunction(id) {
    ...
    switch (id) {
        ...
        case "glory":
            return function (e) { glory(); };
        case "raiseDead":
            elem = document.getElementById("raiseDead100");
            elem.onmousedown = function (e) { raiseDead(100); };
            elem = document.getElementById("raiseDeadMax");
            elem.onmousedown = function (e) { raiseDead(Infinity); };
            return function (e) { raiseDead(); };
        case "summonShade":
            return function (e) { summonShade(); };
        case "pestControl":
            return function (e) { pestControl(); };
        case "grace":
            return function (e) { grace(); };
    }
    return false;
}
